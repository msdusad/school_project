...
// for extrnal file upload file 

if ($_FILES['extrnalfile']['name']!='') {

$file=$_FILES['extrnalfile']['name'];
$tmp_name=$_FILES['extrnalfile']['tmp_name'];
$path="student_docs/video/".$file;
$additional = '1';
$dbfilename=$_FILES['extrnalfile']['name'];
while (file_exists($path)) {
    $info = pathinfo($path);
    $path = $info['dirname'] . '/'
              . $info['filename'] . $additional
              . '.' . $info['extension'];
	$dbfilename=$info['filename'] . $additional. '.' . $info['extension'];
}

$upprocess=move_uploaded_file($tmp_name, $path);
if ($upprocess) {
	//echo "File Uploaded Sucess";
	$media=mysql_query("update student_cv set video1='$dbfilename' where userid='$studentid';");
}

else{

	echo "Error in File Upload".mysql_error();
}
...
<br>
<h1 class="empDbH"><i class="fa fa-check-circle"></i> Welcome to Student dashboard</h1>
...
if($send_media=='yes'){
	...
	if($cv_video!=''){
		?><div class="mediabox">
				<video controls >
                   <source src="../student/student_docs/video/<?php echo $cv_video1;?>" type="video/mp4">
					<source src="../student/student_docs/video/<?php echo $cv_video1;?>" type="video/mogg">
					<source src="../student/student_docs/video/<?php echo $cv_video1;?>" type="video/webm">
</video>
		</div>			<?php
				}
}
...
?>
	 ...
	 <div class="col-md-6" style="margin-left:40%;">
		<form action="application_submitted.php?cover=<?php echo  $cover_letter;?>&media=<?php echo  $send_media;?>&employerid=<?php echo  $empoyer_userid;?>&vacancy_date=<?php echo $vacancy_date;?>" method="post">
			<button class="btn btn-primary" type="submit" name="apply_job" value="">Submit Application</button>
			</form>
		</div>
